...
export default function Settings({ results,userData }) {
                    ...
                    <div className="shadow sm:hidden">
                        <ul role="list" className="mt-2 divide-y divide-gray-200 overflow-hidden shadow sm:hidden">
                            {results.device.map((device) => (
                                <li key={device._id}>
                                    <a href={device.name} className="block px-4 py-4 bg-white hover:bg-gray-50">
                                        <span className="flex items-center space-x-4">
                                            <span className="flex-1 flex space-x-2 truncate">
                                                ...
                                                <span className="flex flex-col text-gray-500 text-sm truncate">
                                                    ...
                                                    <span>
                                                        <span className="text-gray-900 font-medium">{device.amount}</span>{' '}
                                                        {device.currency}
                                                    </span>
                                            ...
                                            {results.device.map((device) => (
                                                <tr key={device._id} className="bg-white">
                                                    <td className="max-w-0 w-full px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                        <div className="flex">
                                                            <a href={device.href} className="group inline-flex space-x-2 truncate text-sm">
                                                                <CashIcon
                                                    ...
                                                    <td className="px-6 py-4 text-right whitespace-nowrap text-sm text-gray-500">
                                                        <span className="text-gray-900 font-medium">{device.description} </span>
                                                    </td>
                                                    ...
                                                    <td className="px-6 py-4 text-right whitespace-nowrap text-sm text-gray-500">
                                                        <time dateTime={device.createAt}>{device.createAt}</time>
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    ...
                                    >
                                        <div className="hidden sm:block">
                                            <p className="text-sm text-gray-700">
                                                ...
                                                <span className="font-medium">{results.device.length}</span> results
                                            </p>
                                        </div>
...
export async function getServerSideProps(context) {
    ...
    if (session?.user.accessToken) {
        ...
        return { props: { componentName:"Setting",session, results: results.data,userData:result2.data  } };
    }
}